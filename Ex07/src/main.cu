...
void printElement_SOA(float4 * , float3 * , int, int  );
//
// Device Functions
//
...
{	
	...
	return sqrt(pow(a.x-b.x,2)+ pow(a.y-b.y,2)  +pow(a.z-b.z,2));// TODO: Calculate distance of two particles
}
...
bodyBodyInteraction(float4 bodyA, float4 bodyB, float3 &force)
{
	...
	if (distance == 0)
		return;
	float c = -GAMMA * bodyA.w * bodyB.w / pow(distance, 2);
	float3 r0 = make_float3(
		(bodyA.x - bodyB.x) / distance,
		(bodyA.y - bodyB.y) / distance,
		(bodyA.z - bodyB.z) / distance
	);  // norm?
...
calculateSpeed(float mass, float3 &currentSpeed, float3 force)
{
	...
	currentSpeed = make_float3(force.x/mass*TIMESTEP,force.y/mass*TIMESTEP,force.z/mass*TIMESTEP);
}
	...
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < numElements; i += blockDim.x * gridDim.x)
	{
		...
		for (int j = 0; j < numElements; j += blockDim.x)
		{
			...
			for (size_t k = 0; k < blockDim.x; k++) {
				...
				bodyBodyInteraction(myPosMass, bodyPosMass, force);
				acc = make_float3(acc.x + force.x, acc.y + force.y, acc.z + force.z);
			}
			__syncthreads();
		}
		bodySpeed[3*i] = acc;
...
updatePosition_Kernel(int numElements, Body_t *bodies)
{
	...
	if (elementId < numElements)
	{
		bodies[elementId].posMass = make_float4(bodies[elementId].posMass.x + bodies[elementId].velocity.x *TIMESTEP,
...
//
__global__ void
updatePosition_Kernel_SOA(int numElements, float4 *bodyPos, float3 *bodySpeed)
{
	int elementId = blockIdx.x * blockDim.x + threadIdx.x;

	if (elementId < numElements)
	{
			bodyPos[elementId]  = make_float4(bodyPos[elementId].x +bodySpeed[elementId].x *TIMESTEP,
			bodyPos[elementId].y +bodySpeed[elementId].y *TIMESTEP,
			bodyPos[elementId].z +bodySpeed[elementId].z *TIMESTEP,
			bodyPos[elementId].w);  
	}
}
...
int main(int argc, char *argv[])
{
	...
	bool report = chCommandLineGetBool("report", argc, argv);
	...
	bool SOA = chCommandLineGetBool("soa", argc, argv);
	 
	float4 h_posMassSOA[numElements];
	float3 h_posSpeedSOA[numElements];
	for (int i = 0; i < numElements; i++)
	{
		h_posMassSOA[i] = make_float4(h_particles[i].posMass.x,h_particles[i].posMass.y,h_particles[i].posMass.z,h_particles[i].posMass.w);  
		h_posSpeedSOA[i] = make_float3(h_particles[i].velocity.x,h_particles[i].velocity.y,h_particles[i].velocity.z);
	}
	...
	//
	float4* d_posMassSOA;
	float3* d_posSpeedSOA;

	cudaMalloc(&d_posMassSOA, static_cast<size_t>(numElements * sizeof(*d_posMassSOA)));
	cudaMalloc(&d_posSpeedSOA, static_cast<size_t>(numElements * sizeof(*d_posSpeedSOA)));
	...
	cudaMemcpy(d_posMassSOA, h_posMassSOA, static_cast<size_t>(numElements * sizeof(float4)), cudaMemcpyHostToDevice);
	cudaMemcpy(d_posSpeedSOA, h_posSpeedSOA, static_cast<size_t>(numElements * sizeof(float3)), cudaMemcpyHostToDevice);
	...
	for (int i = 0; i < numIterations; i++)
	{
		if (SOA)
		{
			sharedNbody_Kernel<<<grid_dim, block_dim, blockSize * sizeof(float4)>>>( numElements,  d_posMassSOA, d_posSpeedSOA);
			updatePosition_Kernel_SOA<<<grid_dim, block_dim>>>( numElements,  d_posMassSOA,  d_posSpeedSOA);
			cudaMemcpy(h_posMassSOA, d_posMassSOA, static_cast<size_t>(numElements * sizeof(float4)), cudaMemcpyDeviceToHost);
			cudaMemcpy(h_posSpeedSOA,d_posSpeedSOA, static_cast<size_t>(numElements * sizeof(float3)), cudaMemcpyDeviceToHost);
			if (!silent)
			{
				printElement_SOA(h_posMassSOA,h_posSpeedSOA, 0, i + 1);
			}
		}
		else
		{
			...
			if (!silent)
			{
				...
				//printElement(h_particles, 1, i + 1);
			}
		}
	...
	{
		std::cout << "\033[31m***" << std::endl
				  << "***ERROR_AFTER_KERNEL_COMPUTE*** " << cudaError << " - " << cudaGetErrorString(cudaError)
				  << std::endl
				  << "***\033[0m" << std::endl;
	...
	{	
		cudaFreeHost(h_posMassSOA);
		cudaFreeHost(h_posSpeedSOA);
		cudaFreeHost(h_particles);
	}
	cudaFree(d_posMassSOA);
	cudaFree(d_posSpeedSOA);
	cudaFree(d_particles);
	
	// Print Meassurement Results
	if(report){
		std::cout <<   numElements << "," << 1e3 * kernelTimer.getTime()<< std::endl;
	}
	else{
...
// Print one element
//
void printElement_SOA(float4 *body_masses, float3 *body_velocities , int elementId, int iteration)
{
	float4 posMass = body_masses[elementId] ;
	float3 velocity =body_velocities[elementId];
