...
main ( int argc, char * argv[] )
{
    ...
    for ( int i = 0; i < optMemCpyIterations; i ++ ) {
        cudaMemcpy(   d_memoryA  , h_memoryA ,  static_cast <size_t> ( optMemorySize ) , cudaMemcpyHostToDevice );
        // H2D copy
    }
    memCpyH2DTimer.stop ();
    ...
    for ( int i = 0; i < optMemCpyIterations; i ++ ) {
        cudaMemcpy(   d_memoryA ,d_memoryB, static_cast <size_t> ( optMemorySize ) , cudaMemcpyDeviceToDevice );
    ...
    for ( int i = 0; i < optMemCpyIterations; i ++ ) {
        cudaMemcpy(   h_memoryB  ,d_memoryB ,  static_cast <size_t> ( optMemorySize ) , cudaMemcpyDeviceToHost );
        // D2H copy
    }
    memCpyD2HTimer.stop ();
